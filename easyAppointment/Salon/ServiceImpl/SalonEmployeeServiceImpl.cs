...
namespace easyAppointment.Salon.ServiceImpl
{
    public class SalonEmployeeServiceImpl : BaseCRUDService< SalonEmployeeResponse,  SalonEmployee,  SalonEmployeeSearchObject,  SalonEmployeeInsertRequest,  SalonEmployeeUpdateRequest>,  SalonEmployeeService
    {
        ...
        public SalonEmployeeServiceImpl(
            ILogger<BaseCRUDService< SalonEmployeeResponse,  SalonEmployee,  SalonEmployeeSearchObject,  SalonEmployeeInsertRequest,  SalonEmployeeUpdateRequest>> _logger,
            EasyAppointmnetSalonDbContext _context,
            IMapper _mapper,
            UserFeignClient userFeignlient) : base(_logger, _context, _mapper)
        {
            _userFeignClient = userFeignlient;
        }
        ...
        public override async Task<List<SalonEmployeeResponse>> Get(SalonEmployeeSearchObject? search = null)
        {
            ...
            {
                var user = await _userFeignClient.GetUserById(response.EmployeeUserId);
        ...
        public virtual async Task<SalonEmployeeResponse> AddSalonEmployee(string username, int salonId)
        {
            ...
            var user = await _userFeignClient.GetUserByUsername(username);
