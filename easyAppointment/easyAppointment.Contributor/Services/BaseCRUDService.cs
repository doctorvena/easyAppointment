...
namespace easyAppointment.Contributor.Services
{
    public class BaseCRUDService<T, TDb, TSearch, TInsert, TUpdate> : ServiceImpl<T, TDb, TSearch> where T : class where TDb : class where TSearch : BaseSearchObject
    {
        ...
        public BaseCRUDService(ILogger<BaseCRUDService<T, TDb, TSearch, TInsert, TUpdate>> logger, EasyAppointmnetUserDbContext context, IMapper mapper) : base(context, mapper)
        {
        }
